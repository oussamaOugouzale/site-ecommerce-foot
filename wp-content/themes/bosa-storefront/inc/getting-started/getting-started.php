<?php
/**
 * Bosa Storefront Notice Handler
 *
 * @package Bosa Storefront
...
function bosa_widgets_backend_enqueue()
{
    ...
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'bosa-storefront' ),
   ...
   /**
        ...
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            echo '<div class="updated notice notice-success is-dismissible getting-started">';
                if ( is_plugin_active( 'advanced-import/advanced-import.php' ) && is_plugin_active( 'keon-toolset/keon-toolset.php' ) && is_plugin_active( 'kirki/kirki.php' ) && is_plugin_active( 'elementor/elementor.php' ) && is_plugin_active( 'breadcrumb-navxt/breadcrumb-navxt.php' ) && is_plugin_active( 'yith-woocommerce-compare/init.php' ) && is_plugin_active( 'yith-woocommerce-quick-view/init.php' ) && is_plugin_active( 'yith-woocommerce-wishlist/init.php' ) && is_plugin_active( 'elementskit-lite/elementskit-lite.php' ) && is_plugin_active( 'woocommerce/woocommerce.php' ) && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'sina-extension-for-elementor/sina-extension-for-elementor.php' ) && is_plugin_active( 'bosa-elementor-for-woocommerce/bosa-elementor-for-woocommerce.php' ) ){
                    echo ('<div class="fs-notice success"><p>'.esc_html__( 'Plugins added successfully. You may dismiss this notice now.', 'bosa-storefront' ).'</p></div>');
                }else{
                    ...
                    echo '<div class="getting-content">';
                        echo ( '<h2>' . sprintf( esc_html__( 'Welcome to %1$s.', 'bosa-storefront' ), $this->theme_name ) . '</h2>' );
                        echo '<h3>To fully take advantage of the best our theme can offer, get started.</h3>';
                        echo ( '<p><a href="#" class="bosa-install-plugins button button-hero button-primary">' . sprintf( esc_html__( 'Get started with %s','bosa-storefront' ), $this->theme_name ) . '</a>' );
                        echo ( '<a href="'.esc_url( admin_url( 'themes.php?page=bosa-info' ) ).'" class="button button-hero button-outline">' . sprintf( esc_html__( '%s Setup','bosa-storefront' ), $this->theme_name ) . '</a></p>' );
                        echo ( '<p class="plugin-notice">'.esc_html__( 'Clicking on get started will install and activate Kirki, Advanced Import, Keon Toolset, Elementor, Bosa Elementor Addons for WooCommerce, Breadcrumb NavXT, WooCommerce, YITH WooCommerce Compare, YITH WooCommerce Quick View, YITH WooCommerce Wishlist, ElementsKit Lite, Sina Extension for Elementor and Contact Form 7.', 'bosa-storefront' ).'</p>' );
                    echo '</div>';   
	...
	public function bosa_getting_started() {
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'bosa-storefront' );
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'bosa-storefront' );
